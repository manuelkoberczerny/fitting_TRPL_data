    ...
    while i < len(len_Data):
        ...
        if len(Data2) != len(np.array(Data['Time'])):
            if len(Data2) > len(np.array(Data['Time'])):
                ...
                Data2 = np.append(Data2,np.zeros(1))
...
def residual2(params, time_fit, Data, limit,  i):
    ...
    Data_fit = np.array(Data[str(i)])[limit]
...
def resid_global(params_ode, time_fit, photo_cond, limit, resid, i):
    ...
    while i < num_rows:
        resid[i,:] = np.abs(residual2(params_ode, time_fit, photo_cond, limit, i))* time_fit
        i += 1
...
resid = np.empty(shape = (num_rows, len(time_fit)))
empty_dat = Data*0
empty_dat[empty_dat == np.nan] = 0

params_ode = Parameters()
for i in range(num_rows):
    params_ode.add(f'tau_{i + 1}', value=600, min=0.1, max=100000, vary=True)
...
for i in range(num_rows):
    R2_val_calc, yfit1 = post_Fitting(results_Model2, Data[str(i)], limit, time_fit, empty_dat, i, nan_beg, nan_end)
    R2_val.append(R2_val_calc)
    Data[str('Fit_' + str(i))] = yfit1
    tau_fit.append(results_Model2.params[f'tau_{i + 1}'].value)
    beta_fit.append(results_Model2.params[f'beta_{i+1}'].value)
    A_parameter.append(results_Model2.params[f'A_{i + 1}'].value)
...
for i in range(num_rows):
    Fit_Values.loc[sample_name[i],'Fluence (nJ cm-2)'] = round(laser_fluence_old[i],2)
